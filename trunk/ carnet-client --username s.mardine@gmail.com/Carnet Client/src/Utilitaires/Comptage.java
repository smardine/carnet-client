...
public class Comptage {
	...
	private void LanceComptage(String string) {
        ...
        if(!directory.exists()){
            ...
            //c'est un dossier, le compteur de fichier est incrementé, celui des dossier est decrémenté
            nbDossier--;
            nbFichier++;
	...
	/**
     * Recupere le nb de fichier present dans un repertoire et ses sous repertoire et l'affiche dans une liste
	...
	private void RecupNbFichierEtAfficheDansUneListe(String directoryPath,DefaultListModel model,JTextField message) {
		...
		if(!directory.exists()){
			...
			for(int i=0 ; i<subfiles.length; i++){
				...
				RecupNbFichierEtAfficheDansUneListe (subfiles[i].toString(),model,message);
			}
