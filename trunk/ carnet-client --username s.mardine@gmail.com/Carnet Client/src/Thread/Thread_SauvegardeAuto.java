...
public class Thread_SauvegardeAuto extends Thread{
	...
	public Thread_SauvegardeAuto(String destination,JProgressBar progress,JLabel operation ){
		...
		if (succesCopie==true){//si les copies ont fonctionnées
			...
			try {
				...
				Historique.ecrire("Archivage du dossier : "+TempDirectory+" vers le chemin : "+EMPLACEMENT);
					...
					JOptionPane.showMessageDialog(null, "Pb lors de la Sauvegarde automatique  : \n\r"+e,
				...
				if (succesZip==true){//l'archivage a reussi, on previens l'utilisateur
					JOptionPane.showMessageDialog(null, "Sauvegarde automatique éffectuée avec succés",
							"Sauvegarde Ok", JOptionPane.INFORMATION_MESSAGE);
					PROGRESSION.setValue(0);
					PROGRESSION.setString(0 + " %");
					MESSAGE_UTILISATEUR.setText("");
					try {
						Historique.ecrire("Sauvegarde automatique éffectuée avec succés");
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
